...
vig_intro   <- paste0("https://cran.r-project.org/web/packages/",
                    "bootf2/vignettes/introduction.html")
vig_simdp   <- paste0("https://cran.r-project.org/web/packages/",
                    "bootf2/vignettes/sim.dp.html")
vig_calcf2  <- paste0("https://cran.r-project.org/web/packages/",
                     "bootf2/vignettes/calcf2.html")
vig_simdpf2 <- paste0("https://cran.r-project.org/web/packages/",
                      "bootf2/vignettes/sim.dp.byf2.html")
vig_bootf2  <- paste0("https://cran.r-project.org/web/packages/",
                     "bootf2/vignettes/bootf2.html")
...
1. `sim.dp()` to simulate dissolution profiles using mathematical models or
    ...
    [Simulation of Dissolution Profiles](`r vig_simdp`) for details.
1. `calcf2()` to calculate similarity factor $f_2$ according to different
    regulatory rules. See vignette 
    [Calculating Similarity Factor $f_2$](`r vig_calcf2`) for details.
1. `sim.dp.byf2()` to find a dissolution profile that, when compared to a given
    ...
    See vignette [Simulation of Dissolution Profiles with Predefined Target 
    $f_2$](`r vig_simdpf2`) for details.
1. `bootf2()` to estimate the confidence intervals of $f_2$s using bootstrap
    method. See vignette 
    [Confidence Intervals of f2 Using Bootstrap Method](`r vig_bootf2`)
    for details.
...
[Introduction to bootf2](`r vig_intro`). 
...
The complete list of arguments are shown below. Read the function manual with
`?calcf2` for more details. In addition, refer to the 
[Introduction to bootf2](`r vig_intro`) for detailed discussion on different
regulatory requirements regarding to the applicability of $f_2$. 
