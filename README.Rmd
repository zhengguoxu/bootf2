...
@Lum-2019-05-WS;@Mandula-2019-05-WS]. Several additional functions were
added later to simulate the dissolution profiles.

There are 4 main functions:

1. `sim.dp()` to simulate dissolution profile using mathematical models
    or multivariate normal distribution. See vignette
    [Simulation of Dissolution Profiles](vignettes/sim.dp.md).
1. `calcf2()` to calculate similarity factor $f_2$ according to different
    regulatory rules. See vignette
    [Calculating Similarity Factor $f_2$](vignettes/calcf2.md).
1. `sim.dp.byf2()` to find a dissolution profile that, when compared to 
    ...
    target $f_2$. See vignette
    [Simulation of Dissolution Profiles with Predefined Target
    $f_2$](vignettes/sim.dp.byf2.md).
1. `bootf2()` to estimate the confidence intervals of $f_2$s using 
    bootstrap method. See vignette
    [Confidence Intervals of $f_2$ Using Bootstrap Method](vignettes/bootf2.md).
...
and some common topics are discussed in the vignette 
...
The complete list of arguments are shown below. Read the package vignette
with `vignette("calcf2", package = "bootf2")` for more details.
...
The minimum required arguments are dissolution profiles for `test` and `ref`.
Data can be read from an Excel file. For interactive use, the `test` and `ref`
can be data frames with the time as the first column and individual units as
the rest columns. The `sim.disso` data frame in the output of `sim.dp()` comes
...
The complete list of arguments are shown below. Read the package vignette
with `vignette("sim.dp.byf2", package = "bootf2")` for more details.
...
will be equal to the target $f_2$. If `target.f2` is specified as a range, 
such as `target.f2 = c(52.05, 53.04)`, then the calculated $f_2$ with simulated
profile will be within this range. 
...
If you don't like the shape of the simulated profile, you can run the function
many times with different `seed` number until you get the shape that you are 
willing to accept. 
...
The complete list of arguments are shown below. Read the package vignette
with `vignette("bootf2", package = "bootf2")` for more details.
